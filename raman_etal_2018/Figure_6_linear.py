...
Objective of this script:
    1) Produce curve of steady state error (i.e. performance) against N, for
     particular gamma and delta t values
        a) in linear case
...
# =============================================================================
    ...
    sizes_to_test = [ [10,5],[16,8],[20,10],[26,13], [30,15],[34,17],[40,20],
                     [50,25], [60,30],[70,35]]
    ...
    for i in range(3):
        ...
        dt_vec.append(1)
    gamma_vec.append(np.asarray([0.07, 1, 0.03]))
    gamma_vec.append(np.asarray([0.06, 1, 0.04]))
    gamma_vec.append(np.asarray([0.05, 1, 0.05]))
    ...
    np.savetxt('linear_x_coords.csv', v1list[0])
    for i in np.arange(3):
        np.savetxt('linear_means_' + str(i+1) +  '.csv', v2list[i])
        np.savetxt('linear_st_errs_' + str(i+1) + '.csv', v3list[i])
